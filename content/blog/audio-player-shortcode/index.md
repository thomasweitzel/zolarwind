...
description = "Three ways to add audio to your blog: native elements, components, and Howler.js."
...
The [Zola](https://www.getzola.org/) theme [Zolarwind](https://github.com/thomasweitzel/zolarwind) provides two components that map to those choices: `audio_simple` uses the native element, `audio` uses a custom player.
...
If you want a consistent player UI across browsers, the `audio` component renders a custom player with JavaScript.
...
I prefer the `audio` component, because I want a stable player UI across browsers.
`audio_simple` stays in the toolbox because it keeps the setup small and uses no extra JavaScript.

## Notes

- The components expect local assets, not remote URLs.
